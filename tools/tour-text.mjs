...
// The gate signs each line by its hash, and only collect() can compute one: hashLine is async and
// lineBlock is not. So sheet() publishes its collected lines here, keyed by the same variant key
// variants() builds, and lineBlock looks the hash up instead of recomputing it.
let byKey = new Map();
...
function lineBlock(line, n, inherit = null, spokenLine = true) {
  ...
  for (const v of shown) {
    ...
    const hash = v.hash ?? byKey.get(v.key)?.hash ?? null;
    if (!usesRuntime(line) && hash) meta.push(`hash \`${hash}\``);
...
function sheet(lines, tot) {
  ...
  byKey = new Map(lines.map((l) => [l.key, l]));
