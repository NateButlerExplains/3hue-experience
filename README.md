...
css/surfaces.css           the rooms' surfaces (O14); loaded by js/surfaces.js only once the tour is on
...
js/screens.js              homography -> matrix3d for the kiosk quad and the rooms' surfaces
js/surfaces.js             the rooms' own surfaces (O14): measured glass and displays drawn in perspective, the tour's writes, door labels
...
content/surfaces.json      the rooms' measured surfaces (O14), merged into geometry.rooms once the tour is on (tools/import-surfaces.mjs)
...
media/voice/               (after the voice render) <guide>/<id>.mp3 + <guide>/<id>.json word timings + manifest.json, see docs/VOICE.md
s/                         share stubs: lobby, win-trust, gain-control, stay-ready, path
tools/                     serve, lint, geometry tool, render pipeline, shares, icons, version stamp, tour-text.mjs (copy sheet), import-surfaces.mjs (room surfaces)
tools/voice/               the voice build (ElevenLabs, or Azure AI Speech; ffmpeg) and its lint; not loaded by the site
...
- each room's surfaces (O14, `content/surfaces.json`, read merged into `geometry.rooms.<door>.surfaces` as the page reads it): passes a and b agree within 2 px on every corner not declared hidden, the quad is their mean within 0.5 px there, the quad maps to a `matrix3d`, its station is one of the door's, its occluders and text box stay on the image, and `measuredOn.sha256` is the SHA-256 of the shipped `media/rooms/<door>-2560.jpg` (a new render fails until it is measured again)
...
- **The rooms talk back (O14).** In a door or station scene the room's own glass and displays (`js/surfaces.js`) show what the guide is saying. A node may carry `write: {<surface id>: entry | null}` (its base state) and so may any line; an entry is `{kind: sign | list | card | status | glow, lines: [template | {ref}], at?}`. What a surface shows is the fold of the node's base write and every shown line's write up to the line on screen (`js/tourtext.js` `foldWrites`), recomputed on every render, so Previous, deep links, resizes and the trigger versions need no state of their own and nothing carries into the next node. With the voice on, an entry with `at` waits for that word (the voice's `lobby:voice` word events carry `{line, index, word}`); otherwise it shows as its line starts. `cue: {surface}` frames that surface (`js/stage.js` `frameRoom`; a phone zooms past the room's 1.6× cap so the surface fills its strip) and marks it; a station scene or cue marks and frames the surfaces standing for that station. A click on a lit surface goes back to the first line that writes or cues it. Surfaces are aria-hidden and pointer-only; the card carries a screen-reader list, "The room shows: …" (`strings.tourRoomShows`), of every lit surface's words. Type is fitted, hidden below 12 CSS px, and printed on glass over a feathered backlight so it keeps 4.5:1. While the tour is on (`?tour=1`, or the gate approved) the door view's surfaces replace the station pins, carrying each station's label and opening its section; with the tour off the pins stay, and `?room-preview=` always keeps the pins. `tourState()` reports `surfaces: {<id>: words | kind}`; `window.__lobby.surfaces` and `.roomFit` give the drawn state.
- **The voices.** Avi and Huey are ElevenLabs voices (O13), rendered at build time into `media/voice/<guide>/` by `tools/voice/` (`npm run voice:dry` prices it per guide with no key; `node tools/voice/build.mjs --check` tests the key and both voices; `npm run voice` renders). A line pinned with `who` is rendered in its guide's voice only; every other line and every Ask answer in both. The key is `ELEVENLABS_API_KEY` in `~/.env` (the misspelt `ELEVNLABS_API_KEY` is read too); Azure AI Speech stays available per guide. The runtime switch is `guide.voice.required`: while it is false (today, no render committed) the tour is captions-only and never requests `media/voice/`. Everything else, credentials and costs included, is in `docs/VOICE.md`.

## Geometry and the geometry tool

`content/geometry.json` holds every position in pixels of the 2880x1621 plate (`plate.reference` 1672 is the width of the original concept the first numbers were converted from). Keys: `layout` (header heights, sheet top, dolly zoom/ms/ease, pin fade, pulse period, room fade timings, heading sizes), `doorways.<id>` (`center`, `frame` `[l,t,r,b]`, `sign`), `tower`, `rings[]` (`stage`, `label`, `arc` SVG path), `stairPill`, `phoneBand`, `kiosk` (`measurements.a`/`.b`, `quad`, `focus`), `logoMark` (written by the icon tool), `rooms.<door>` (`width`, `height`, `focus {x,y}` and `stations {id: [x,y]}` as fractions of the room image; `surfaces` and `measuredOn` come from `content/surfaces.json`, below) and `measured` (per-item flag: seeded from the 1672 conversion until re-measured in the tool).

**Room surfaces (O14).** `content/surfaces.json` holds, per door, what `geometry.rooms.<door>` takes on once the tour is on: `js/surfaces.js` fetches it (and `css/surfaces.css`) the first time a room mounts its surfaces, or at idle with the tour on, and merges it into the geometry; the lint merges it the same way (`readGeometry` in `tools/check-manifest.js`). It is its own file because `content/geometry.json` is preloaded at boot, and 17 KB of polygons there cost the gated lobby its first-paint budget (P2a-D05) for something it does not show. `rooms.<door>.surfaces.<id>` holds each measured surface in pixels of the 2560×1440 master (the same pixels as `media/rooms/<door>-2560.*`; `js/surfaces.js` scales them to whichever derivative is on screen): `role` (glass, screen, scrim, region, dial: how it carries light and type), `station` (the door station it stands for in the door view and in station cues), `target` (whether a pointer may pick it), `hidden` (corner indexes that were rebuilt, not read), `quad` (TL TR BR BL, the mean of the two passes), `measurements.a` and `.b` (the passes), `text` (the writable box as insets, or null), `panes`, `dial` (the clock: centre and radii; its quad is the ellipse's bounding box) and `occluders` (polygons of what stands in front). `rooms.<door>.measuredOn` is `{file, sha256}` of the render they were measured on. `node tools/import-surfaces.mjs [dir]` writes the file from the measurement files (`art/surfaces/<door>.json` of the main checkout by default: two passes per surface, occluder polygons and notes) and records the station mapping and the text boxes; the lint re-checks it. Win Trust: panels 1-2 urgency, 3-4 gap, 5-6 program, the desk monitor proof, the folders services, the niche decision. Gain Control: the wall display urgency, the left amber panel gap, the right one program, the model proof; the console under it is a glow only, and no surface stands for services or decision. Stay Ready: the clock urgency, the three monitors gap, the rack services, the three panels proof, program and decision.

`tools/geometry-tool.html` must be opened through the local server (`/3hue-experience/tools/geometry-tool.html`) because it fetches `../content/geometry.json`. It shows a picture at native size with a zoom slider (0.2-8×), draws the current geometry, and reads pixels from clicks. The Picture picker takes the 2880 lobby plate or a room's 2560 master (`media/rooms/<door>-2560.jpg`). On the plate the modes are Point, Rect (2 clicks), Quad (TL TR BR BL, with a live `matrix3d` preview) and Arc (left, apex, right), and "Assign to" writes the measurement into the chosen key. On a room the tool edits the room's surfaces (loaded from `content/surfaces.json`): pick a surface (or add one by id), its role, station and whether it is a pointer target; a Quad goes to pass a or b of that surface, and once both passes agree within 2 px on every corner not ticked hidden the quad becomes their mean (and `measuredOn` takes the render's SHA-256); Polygon mode collects points that "Close polygon" adds as an occluder. Every surface is drawn with its passes, its occluders and a grid through its matrix. "Copy JSON" copies the edited geometry for you to paste back into `content/geometry.json`, and "Copy surfaces JSON" the rooms' surfaces for `content/surfaces.json` (the tool never writes a file). The "50% overlay of the 1672 original" checkbox needs the git-ignored `tools/reference/00-original-1672x941.png` (fork path in `docs/PROVENANCE.md`). Room focus and station fractions in `geometry.rooms` are still entered by hand.
...
8. Surfaces (O14). Measure the new render's surfaces twice (the geometry tool's room mode, or the measurement files under `art/surfaces/` and `node tools/import-surfaces.mjs`). The lint compares `rooms.<door>.measuredOn.sha256` in `content/surfaces.json` with the shipped `media/rooms/<door>-2560.jpg`, so a re-rendered room fails until its surfaces are measured on it.
9. Wire. Set `doors[].room.render` to the base path `media/rooms/<door>` (no size or extension; `js/rooms.js` picks 1280/2048/2560 from viewport x device pixel ratio and AVIF/WebP/JPEG from a format probe) and add `geometry.rooms.<door>` with `width`, `height`, `focus` and `stations`. Run the lint. Recapture that door's share card.
...
| `?room-preview=<relative image path>` | mounts that file as the room for any door opened; used with `#/door/<id>` for the render gate (its surfaces are not measured, so the pins show) |
...
| `?tour=1` | the walk button starts the guided tour (and `#/tour/<node>` links open it) before `tour.gate` is `approved`; the preview. The door view's rooms then carry their surfaces in place of the pins (O14) |
