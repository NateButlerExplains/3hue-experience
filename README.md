...
`npm run lint` also lints the tour script (`tour.manifest`, or `node tools/check-manifest.js --tour <file>` for another): the graph (targets resolve, every node reachable, no loop without a choice, no dead end), refs and tokens that resolve, figures only through sourced manifest fields (spelled-out numbers count), no door title, buyer label or stage name typed into the script, at most 4 options per choice, label lengths, a source and status on every text line and Ask answer, and the same vocabulary and brand checks. Then `tools/voice/lint.mjs` checks `media/voice/` against the script: missing or stale audio is a warning while `guide.voice.required` is false and an error once it is true; a broken file is always an error.
