...
`npm run lint` also lints the tour script (`tour.manifest`, or `node tools/check-manifest.js --tour <file>` for another): the graph (targets resolve, every node reachable, no loop without a choice, no dead end), refs and tokens that resolve, figures only through sourced manifest fields (spelled-out numbers count), no door title, buyer label or stage name typed into the script, at most 4 options per choice, label lengths, a source and status on every text line and Ask answer, and the same vocabulary and brand checks, and the rooms' writes (O14): `cue: {surface}` and every `write` key name a surface of the node's door (a door or station scene with a named door; `@` never carries a surface id), entries are `{kind, lines, at?}` with at most four items (two for a sign, none for a glow, a ref for a card), items pass the caption rules or resolve as refs, nothing is written on a surface with no text box, and `at` is a word of its line's caption for every door and guide. Then `tools/voice/lint.mjs` checks `media/voice/` against the script: missing or stale audio is a warning while `guide.voice.required` is false and an error once it is true; a broken file is always an error.
